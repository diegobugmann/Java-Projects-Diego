...
public class LotteryController {
	...
	public LotteryController(LotteryModel model, LotteryView view) {
		...
		view.getTipArea().addBtn.setOnAction(e -> {
			if (model.getTipList().getTips().size() < LotteryModel.MAX_TIPS) model.getTipList().addTip(new Combination());
		});
		...
		view.getTipArea().getTipTable().getItems().addListener((ListChangeListener<Combination>)(c -> {
			...
			view.getDrawArea().getCurrentTipInfoLabel().setText("Anzahl Tipps: "+model.getTipList().getTips().size()+
					"\nAktuelle Kosten: "+(dm.format(currentPrice)));
		})); //update the current costs and number of tips whenever a Tip is added/deleted
	...
	//if the number is valid and doesn't already exist in this combination, update it!
	private void createEditCellEvents() {
		...
		view.getTipArea().getTipTable().getColSupNum().setOnEditCommit(e -> {
			String newValue = e.getNewValue();
			if (SuperNumber.isValid(newValue)) e.getRowValue().getSupNum().setNumber(Integer.parseInt(newValue));
			else e.getTableView().refresh();
		});
	}
}